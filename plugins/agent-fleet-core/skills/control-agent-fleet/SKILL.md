...
Fleet Specはdesired state、SQLiteは実行時のlogical stateとして分ける。Fleetは`fleet.harness/v3`を使い、各メンバーには、役割と分離した`runtime.product`、`runtime.command`、`runtime.model`、`runtime.effort`、`runtime.fallback`を必ず指定する。`spec.view_profile`は任意のViewProfileファイルパスで、省略時はHerdr pluginの既定値を使う。pane ID、workspace ID、tab IDや観測geometryはFleetにもCore DBにも保存しない。
...
Herdr Adapterへは版付きの公開JSON契約だけを渡す。相手pluginの導入先を探索・importしない。実行環境や表示の操作が必要なら`agent-fleet-herdr`を別途使い、Fleet YAMLの絶対パスを統合入口へ渡す。
