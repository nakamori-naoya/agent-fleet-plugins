...
| reviewや検証のtaskの依存 | 対象のtaskを`depends_on`に入れない。`depends_on`は対象の受理を求めるが、受理はreviewの後に来るからである。managerが対象の`reported`を見てから割り当てる |
...
止まるのは、toolが拒否したか、契約外の操作が要るときである。

- `validate_fleet.py`またはCore CLIが終了code非0を返した。拒否理由をそのまま返し、状態を変えない。
...
止まるときは、実行した操作、CLIの`error`、変えていない状態、再開に必要な修正を返す。

判断の揺れでは止まらない。Fleet Specの役割分担や依存順序の妥当性に疑問があっても、Specが検査を通るなら宣言どおりに操作し、疑問点は成果とともに仮説として示す。`task.report`の終端報告に含める検証結果や停止理由が不足していると感じたら、確認できた範囲と未確認の範囲を分けて書き、推測で補わない。
