...
```gherkin
Scenario: 起動済み艦隊がプラグイン更新後も同じHookで作業を続ける
  Given 艦隊が内容ハッシュAの役割Hookで起動済みである
  And プラグインの役割Hookが内容ハッシュBへ更新されている
  When 利用者が同じ艦隊の起動コマンドを再実行する
  Then 艦隊制御プロセスは内容ハッシュAの固定版を検査する
  And 既存paneは内容ハッシュAの固定版を使い続ける
  And 新しいpaneを追加しない
```

```gherkin
Scenario: 停止後の再起動で新しいHookへ切り替える
  Given 艦隊が内容ハッシュAの役割Hookで停止済みである
  And プラグインの役割Hookが内容ハッシュBへ更新されている
  When 利用者が艦隊を再起動する
  Then fleet-runtimeは内容ハッシュBの固定版を作る
  And Herdrは各paneへ内容ハッシュBの固定配置先を渡す
  And Coreは以前の役割文脈確認を無効にする
```
...
通常の監視では、Coreの公開操作`task.list`を使う。この操作はタスクの担当、状態、期待する成果、完了条件、最新の状態報告、最新の進捗報告、次回報告予定、報告期限状態だけを返す。完了時の状態報告はタスク受入後も読める。マネージャーはCore SQLiteを直接読まず、艦隊全体の大きなJSONを`jq`などで加工して監視結果を作らない。これにより、実行環境ごとのcommand配置に依存せず、Coreが保証した同じ形の事実だけで受入判断できる。

| 状態一覧の区分 | 内容 |
|---|---|
| 艦隊 | 艦隊ID、目的、最終更新時刻 |
| メンバー | 論理エージェントID、役割、稼働状態 |
| タスク | 担当、状態、期待する成果、完了条件、最後の状態報告、最後の進捗報告、次回報告予定 |
...
Scenario: マネージャーがCoreの公開操作から進捗を判断する
  ...
  Then Coreは担当、状態、期待する成果、完了条件、最新の状態報告、最新の進捗報告、次回報告予定、報告期限状態を返す
...
flowchart LR
    F["艦隊設定<br/>誰が何を担うか"]
    R["版固定参照<br/>local/development-focus@1"]
...
艦隊設定には目的、メンバー、担当候補、完了条件、停止条件に加え、各メンバーが使うAI製品、モデル、思考量、代替モデル方針を置く。実際のpane番号、workspace番号、比率、設定ファイルのpathは置かない。表示プロファイルにはマネージャー領域とその他領域の重み、分割方向、対応人数を置き、艦隊IDやpane番号を置かない。
...
別の利用者設定directoryを使う場合だけ`--fleet-dir`と`--profile-dir`を指定する。起動時の作業directoryは現在位置とする。AI製品とモデルは起動commandで艦隊全体へ外付けせず、各メンバーの`runtime`から解決する。比率を一時引数で上書きせず、再現可能な新しい表示プロファイル版として保存する。
