...
Codexでは、プラグインを有効にしただけではHookは信頼済みにならない。通常のCodexでは`/hooks`から内容を確認して信頼する。艦隊では、メンバーごとの確認待ちで役割同期が止まらないよう、艦隊設定の`spec.runtime.codex_hook_trust`を`preapproved`に明示すると、Herdrから起動するCodexだけへ`--dangerously-bypass-hook-trust`を渡す。省略時は安全側の`review`であり、プラグイン更新で旧実行ファイルが消えた場合も未確認の新版へ自動移行しない。この設定はHookの信頼確認だけを省略し、tool承認やsandboxは無効にしない。Claude Codeのコマンド型Hookは時間切れ時に出力が破棄され、入力自体は処理されるため、Hook単体を安全境界にせず、Core側の配送制限を併用する。
