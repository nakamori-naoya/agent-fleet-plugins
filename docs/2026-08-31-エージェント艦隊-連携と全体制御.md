...
通常セッションでのHook無効化は信頼方針とは別である。Claude Codeは艦隊起動時だけHerdr package内のHook sidecarを`--plugin-dir`で読む。Codexは個別のHook pluginをinstallせず、通常時はHerdr packageを無効にし、艦隊操作時だけ`--config plugins.agent-fleet-herdr@agent-fleet.enabled=true`で有効にする。
...
Scenario: Codexの役割Hook登録がない間はCodexの作業枠を作らない
  ...
  And Codexにagent-fleet-herdr packageが登録されていない
