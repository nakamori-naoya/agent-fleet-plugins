...
for manifest in \
  ...
  jq -e '.version=="0.4.1" and (.name=="agent-fleet-core" or .name=="agent-fleet-herdr")' "$manifest" >/dev/null || failed=1
done
for manifest in "$HOOK_PLUGIN/.codex-plugin/plugin.json" "$HOOK_PLUGIN/.claude-plugin/plugin.json"; do
  jq -e '.version=="0.4.1" and .name=="agent-fleet-session-hooks"' "$manifest" >/dev/null || failed=1
done
...
jq -e '.name=="agent-fleet" and (.plugins|length==3) and ([.plugins[].name]|sort)==["agent-fleet-core","agent-fleet-herdr","agent-fleet-session-hooks"] and all(.plugins[]; .version=="0.4.1")' \
